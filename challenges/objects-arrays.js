...
for(grad in graduates) {
  contactInfo.push(`${graduates[grad].first_name} ${graduates[grad].email}`);
}
...
let names = zooAnimals.forEach(function(x){
  displayNames.push(`Animal Name: ${x.animal_name}, Scientific Name: ${x.scientific_name}`);
  }
)
...
lowCaseAnimalNames = zooAnimals.map(object1 => object1.animal_name.toLowerCase());
...
let lowPopulationAnimals = zooAnimals.filter((larger) => {return larger.population < 5;
});
...
// I did that already but it's just how I wrote it.
